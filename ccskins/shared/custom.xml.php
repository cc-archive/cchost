...
<?if ( !empty($A['advanced_search_url'])) {?><a  href="<?= $A['advanced_search_url']?>"><?= $GLOBALS['str_advanced'] ?></a><?}?></div>
</form>
<?}
function _t_custom_Ratings_Chart($T,&$A) {
  ?><p ><?= $GLOBALS['str_highest_rated'] ?></p>
...
<?if ( !($A['chart']) ) {?><p ><?= $GLOBALS['str_no_chart'] ?></p><?}}
function _t_custom_Editorial_Picks($T,&$A) {
  $A['latest_title'] = $GLOBALS['str_editors_picks'];$A['latest_tag'] = 'editorial_pick';$T->Call('custom.xml/_query_latest');
?><a  href="<?= $A['home-url']?>editorial/picks" class="cc_more_menu_link"><?= $GLOBALS['str_see_all_picks'] ?>...</a>
<?}
function _t_custom_Latest_Uploads($T,&$A) {
  $A['latest_title'] = $GLOBALS['str_new_uploads'] ;$A['latest_tag'] = '';$T->Call('custom.xml/_query_latest');
}
function _t_custom_Latest_Remixes($T,&$A) {
  $A['latest_title'] = $GLOBALS['str_new_remixes'] ;$A['latest_tag'] = 'remix';$T->Call('custom.xml/_query_latest');
}
...
<a  href="<?= $A['home-url']?>playlist/browse" class="cc_more_menu_link"><?= $GLOBALS['str_more_playlists'] ?>...</a>
<?}
