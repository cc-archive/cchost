...
function _t_playlist_playlist_popup($T,&$A) {
  $carr101 = $A['args']['with'];$cc101= count( $carr101);$ck101= array_keys( $carr101);for( $ci101= 0; $ci101< $cc101; ++$ci101){    $A['R'] = $carr101[ $ck101[ $ci101 ] ];   ?><a href="<?= $A['home-url']?>api/playlist/remove/<?= $A['args']['upload_id']?>/<?= $A['R']['cart_id']?>" class="cc_playlist_menu_item"><span><?= $GLOBALS['str_remove_from']?> <span class="cc_playlist_name"><?= $A['R']['cart_name']?></span></span></a>
<?}$carr102 = $A['args']['without'];$cc102= count( $carr102);$ck102= array_keys( $carr102);for( $ci102= 0; $ci102< $cc102; ++$ci102){    $A['R'] = $carr102[ $ck102[ $ci102 ] ]; ?><a href="<?= $A['home-url']?>api/playlist/add/<?= $A['args']['upload_id']?>/<?= $A['R']['cart_id']?>" class="cc_playlist_menu_item"><span><?= $GLOBALS['str_add_to']?> <span class="cc_playlist_name"><?= $A['R']['cart_name']?></span></span></a>
<?}?><a href="<?= $A['home-url']?>api/playlist/new/<?= $A['args']['upload_id']?>" class="cc_playlist_menu_item cc_playlist_add_mi"><span><?=$GLOBALS['str_add_to_new_playlist']?></span></a>
<?}


function _t_playlist_playlist_browse($T,&$A) {
  $carr103 = $A['records'];$cc103= count( $carr103);$ck103= array_keys( $carr103);for( $ci103= 0; $ci103< $cc103; ++$ci103){    $A['PL'] = $carr103[ $ck103[ $ci103 ] ];   ?><div  class="cc_playlist_line" id="_pl_<?= $A['PL']['cart_id']?>"><?= $A['PL']['cart_name']?> <span class="cc_playlist_dyn_user"><?= $GLOBALS['str_created_by'] ?>
...
<?if ( !($A['PL']['cart_dynamic']) ) {?><span> $GLOBALS['str_items'] : <?= $A['PL']['cart_num_items']?></span><?}?></div>
...
<?= $GLOBALS['str_created_by']?> <a target="_parent" href="<?= $A['R']['artist_page_url']?>"><?= $A['R']['user_real_name']?></a> on <?= $A['R']['cart_date_format']?>
      </span>
...
<a class="cc_playlist_button" href="javascript://playlist_menu_<?= $A['item']['upload_id']?>"><span><?=$GLOBALS['str_add_to']?> ...</span></a>
...
<span class="cc_playlist_date">
      <?= $GLOBALS['str_created_by']?> <a target="_parent" href="<?= $A['R']['artist_page_url']?>"><?= $A['R']['user_real_name']?></a>
...
<link  rel="stylesheet" type="text/css" href="<?= $T->URL('css/detail.css') ?>"  title="Default Style"></link>
<script  src="<?= $T->URL('js/playlist.js') ?>" ></script>
<div  id="playlist_browser" class="grid"><span><?= $GLOBALS['str_getting_playlists'] ?>...</span></div>
