...
class CCPoolSources extends CCPoolTree
{
    ...
    {
        $r =& $this->_get_relatives($record['upload_id'],false,empty($record['works_page']));
        return $r;
    }
...
{
    function CCPoolRemixes()
    {
        $this->CCPoolTree('pool_tree_pool_child', 'pool_tree_parent');
    }

    function & GetApprovedRemixes($record)
    {
        $r =& $this->_get_relatives($record['upload_id'],true,!empty($record['work_page']));
        return $r;
    }
    ...
    function & GetUnapproved()
    {
        ...
        $j3 = $this->AddJoin( new CCUsers(),     'upload_user' );
        $j4 = $this->AddJoin( new CCPools(),     'pool_item_pool' );

        $rows = $this->QueryRows( 'pool_item_approved < 1' );
    ...
    function HasUnapproved()
    {
        $j1 = $this->AddJoin( new CCUploads(),    'pool_tree_parent' );
        $j2 = $this->AddJoin( new CCPoolItems(), 'pool_tree_pool_child' );
        $count = $this->CountRows( 'pool_item_approved < 1' );
        $this->RemoveJoin($j1);
        $this->RemoveJoin($j2);
        return( $count > 0 );
    }
...
class CCLocalPoolSources extends CCRemixTree
{
    ...
    {
        $r =& $this->_get_relatives($item['pool_item_id'],empty($item['works_page']));
        return $r;
    }
...
{
    function CCLocalPoolRemixes()
    {
        $this->CCRemixTree('pool_tree_child', 'pool_tree_pool_parent', 'cc_tbl_pool_tree');
    }

    function & GetRemixes($pool_item)
    {
        $r =& $this->_get_relatives($pool_item['pool_item_id'], empty($pool_item['works_page']) );
        return $r;
    }
...
class CCPool
{
    function PoolQuery($pool_id, $query)
    {
        ...
        if( substr($api_url,0,7) == 'http://' )
        {
            ...
            global $MAGPIE_PARSER;
            $MAGPIE_PARSER = 'CCMagpieParser';
            $rss = CCFeeds::ReadFeed( $query_url );
            if( !empty($rss->ERROR) )
            {
        ...
        {
            $obj = new $api_url();
            $retval = array( 'pool_items', $obj->LocalSearch($pool_id,$query) );
        }
    ...
    function AddPool($pool_site_api_url)
    {
        ...
        $args['pool_description']  = empty($C['description']) ? "Pool pool of samples at $host" : $C['description'];
