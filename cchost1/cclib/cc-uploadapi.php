 ...
 //    LEFT OUTER JOIN cc_tbl_license  c ON upload_license  = c.license_id
    ...
    */
    function CCUploads()
    {
        ...
        if( !CCUtil::IsHTTP() || !CCUser::IsAdmin() )
        {
            $this->_filter .= " ( ";
...
class CCFiles extends CCTable
{
    ...
    *
    * @param mixed $row_or_id An upload_id or a record from CCUploads table
    * @returns array $rows Returns array of file records
    */
    function FilesForUpload($row_or_id)
    {
        if( is_array($row_or_id) )
        {
            $row_or_id = $row_or_id['upload_id'];
        }

        $where['file_upload'] = $row_or_id;
