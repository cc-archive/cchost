...
/*
        $fields = array(
                'contest_delete' => array (
                        'label'      => 'Delete this contest',
                        'form_tip'   => 'Permanantly remove the contest, sources and entries',
                        'value'      => 0,
                        'formatter'  => 'checkbox',
                        'flags'      => CCFF_NOUPDATE ),
            );
...
class CCContest
{
    ...
    */
    function CreateContest()
    {
        ...
        else
        {
            ...
            $pages['contest'] = CCContest::_get_contest_tabs($contest_short_name, $long_name );
            ...
            CCUtil::SendBrowserTo( ccr($contest_short_name) );
        }
    ...
    function _get_contest_tabs($short_name, $full_name )
    {
        $a = array(
             ...
             'entries' =>     array(  'text'   => cct('Entries'),
                                   ...
                                   'limit'  => 1,
                                   'tags'   => CCUD_CONTEST_ENTRY,
                                   'access' => CC_DONT_CARE_LOGGED_IN,
                                   'function' => 'any' ),

             'submit'   =>    array(  'text'   => cct('Submit'),
             ...
             'submitsources'   =>    array(  'text'   => cct('Submit Sources'),
                                   'help'   => cct('Submit sources for the ') . $full_name . ' contest',
                                   'tags'   => '/contest/submitsource/' . $short_name,
                                   'limit'  => 1,
                                   'access' => CC_ADMIN_ONLY,
                                   'function' => 'url' ),

             'edit'   =>    array(  'text'   => cct('Properties'),
                                   'help'   => cct('Edit the properties (dates, rules, etc.) for ') . $full_name . ' contest',
    ...
    */
    function EditContest($contest_short_name)
    {
        ...
        $record = $contests->GetRecordFromShortName($contest_short_name) ;
        CCPage::SetTitle(cct('Edit ') . $record['contest_friendly_name']);
    ...
    */
    function SubmitSource($contest_short_name)
    {
        ...
        CCPage::SetTitle(cct('Submit Contest Sources'));
        $contests =& CCContests::GetTable();
        $record =& $contests->GetRecordFromShortName($contest_short_name);
        ...
        if( !empty($_POST['uploadcontestsource']) )
        {
            ...
            if( $form->ValidateFields() )
            {
                ...
                $relative_dir = $this->_get_upload_dir($record);
    ...
    */
    function & _contest_uploads($contest_short_name,$systags)
    {
    ...
    * @param string $systags System tag to filter on
    */
    function _list_contest_uploads($contest_short_name,$systags)
    {
        $records =& $this->_contest_uploads($contest_short_name,$systags);
