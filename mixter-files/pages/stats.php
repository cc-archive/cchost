...
<?
$A['total_uploads'] = cc_stats_total_uploads();
$A['total_remixes'] = cc_stats_total_uploads('remix');
$A['total_pells'] = cc_stats_total_uploads('acappella');
$A['total_orgs'] = cc_stats_total_uploads('original');
$A['total_samps'] = cc_stats_total_uploads('sample');
$A['rtotal_pells'] = cc_stats_percent_remixed('acappella');
$A['rtotal_orgs'] = cc_stats_percent_remixed('original');
$A['rtotal_samps'] = cc_stats_percent_remixed('sample');
...
$carr101 = cc_stats_most_remixed();
$cc101= count( $carr101);
$ck101= array_keys( $carr101);
for( $ci101= 0; $ci101< $cc101; ++$ci101)
{ 
   $A['smpld'] = $carr101[ $ck101[ $ci101 ] ];
   
?><tr ><td ><a  class="cc_user_link" href="<?= $A['smpld']['artist_page_url']?>"><?= $A['smpld']['user_real_name']?></a></td>
...
$carr102 = cc_stats_uploads_by_month();
$cc102= count( $carr102);
$ck102= array_keys( $carr102);
for( $ci102= 0; $ci102< $cc102; ++$ci102)
{ 
   $A['mostat'] = $carr102[ $ck102[ $ci102 ] ];
   
?><tr ><td ><?= $A['mostat']['month']?>:</td><td ><?= $A['mostat']['c']?></td></tr>
<?
} // END: for loop
...
$carr103 = cc_stats_signups_by_month('2004-09');
$cc103= count( $carr103);
$ck103= array_keys( $carr103);
for( $ci103= 0; $ci103< $cc103; ++$ci103)
{ 
   $A['mostat'] = $carr103[ $ck103[ $ci103 ] ];
...
$carr104 = cc_stats_most_of_type('remix');
$cc104= count( $carr104);
$ck104= array_keys( $carr104);
for( $ci104= 0; $ci104< $cc104; ++$ci104)
{ 
   $A['a'] = $carr104[ $ck104[ $ci104 ] ];
   
?><tr ><td ><a  href="<?= $A['a']['artist_page_url']?>" class="cc_user_link"><?= $A['a']['user_real_name']?></a></td><td ><?= $A['a']['c']?></td></tr>
<?
} // END: for loop
...
$carr105 = cc_stats_most_of_type('editorial_pick',7);
$cc105= count( $carr105);
$ck105= array_keys( $carr105);
for( $ci105= 0; $ci105< $cc105; ++$ci105)
{ 
   $A['a'] = $carr105[ $ck105[ $ci105 ] ];
...
$carr106 = cc_stats_remixes_of_type('acappella');
$cc106= count( $carr106);
$ck106= array_keys( $carr106);
for( $ci106= 0; $ci106< $cc106; ++$ci106)
{ 
   $A['pellrmx'] = $carr106[ $ck106[ $ci106 ] ];
   
?><tr ><td ><a  class="cc_file_link" href="<?= $A['pellrmx']['file_page_url']?>"><?= CC_strchop('$pellrmx/upload_name',30)?></a></td>
...
?></table>
<table  class="statstable light_bg dark_border">
<tr ><th  class="shead dark_bg light_color" colspan="2">Licenses for Samples</th></tr>
<?

$carr107 = cc_stats_lics('sample');
$cc107= count( $carr107);
$ck107= array_keys( $carr107);
for( $ci107= 0; $ci107< $cc107; ++$ci107)
{ 
   $A['Rsx'] = $carr107[ $ck107[ $ci107 ] ];
   
?><tr ><td ><?= $A['Rsx']['license_name']?></td><td ><?= $A['Rsx']['cnt']?></td></tr>
<?
} // END: for loop

?></table>
<table  class="statstable light_bg dark_border">
<tr ><th  class="shead dark_bg light_color" colspan="2">Licenses for A Cappellas</th></tr>
<?

$carr108 = cc_stats_lics('acappella');
$cc108= count( $carr108);
$ck108= array_keys( $carr108);
for( $ci108= 0; $ci108< $cc108; ++$ci108)
{ 
   $A['Rp'] = $carr108[ $ck108[ $ci108 ] ];
   
?><tr ><td ><?= $A['Rp']['license_name']?></td><td ><?= $A['Rp']['cnt']?></td></tr>
<?
} // END: for loop
