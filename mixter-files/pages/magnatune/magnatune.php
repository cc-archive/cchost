<?
if( !defined('IN_CC_HOST') )
    die('Welcome to ccHost');

function _t_magnatune_init($T,&$targs) {
    
}
?><style >
p { font-size: 12px; }
.contestbox 
{
  margin-top:10px;
  margin-bottom:30px;
}
.contestback 
{
  float:left;
  background-color:#eee;
  border:1px solid #ddd;
  width:100%;
}
</style>
<h1 >Creative Commons Magnatune Lisa DeBenedictis Remix Contest</h1>
<h2  style="color: brown;font-size:18px;margin:0px;font-weight:normal;padding:5px;
           text-align:center; border: 1px solid brown;margin:7px;">Hear the <a  href="<?= $A['root-url']?>magnatune/view/contest/winners">winners</a>!</h2>
<img  src="/mixter-files/ccdj2.gif" height="150" style="float:right;margin-left:10px;margin-bottom:5px;" />
<h2  style="font-size:18px;margin:0px;font-weight:normal;">What it was:</h2>
<p >A contest to remix two vocal tracks ("Cuckoo" and/or "Below") from Lisa DeBenedictis's <a  href="http://magnatune.com">Magnatune</a> release "<a  href="http://magnatune.com/artists/debenedictis">Tigers</a>" and using samples from the entire catalog of <a  href="http://creativecommons.org/">Creative Commons</a> licensed tracks on <a  href="http://magnatune.com/">Magnatune</a> (over 1,700 tracks to sample from!)</p>
<p >The CC Magnatune/Lisa DeBenedictis Remix Contest built upon the label's catalog featuring tracks licensed under the <a  href="http://creativecommons.org/licenses/by-nc-sa/2.0">Attribution-NonCommerical-ShareAlike licenses</a>, giving you the right to rip, sample, mash, and share, without legal hassle. </p>
<div  class="contestbox">
<div  class="contestback" style="width:370px;float:right;margin:8px;">
<div  style="padding:10px 20px 10px 10px;">
<a  href="/contests/magnatune"><img  src="/mixter-files/magnatune_logo.gif" alt="Magnatune" style="float:right;margin-left:10px;margin-bottom:6px;border:1px solid #ccc;" /></a>
<a  href="http://magnatune.com/info"><h2  style="font-size:18px;margin:0px;font-weight:normal;">What's Magnatune?</h2></a>
<br  clear="right" />
<p >"Musicians need to be in control and enjoy the process of having their music released. The systematic destruction of musician's lives is unacceptable: musicians are very close to staging a revolution (and some already have)."</p>
<p ><i >John Buckman, CEO Magnatune</i></p>
<p ><a  href="http://magnatune.com/info">Read more about this innovative music label.</a></p>
</div>
</div>
</div>
<h2  style="font-size:18px;margin:0px;font-weight:normal;">Materials:</h2>
<p >Even though the contest is officially over, the Magnatune catalog is still online. You can find all of the source material, including the official Lisa DeBenedictis a cappellas as well as
100s of pre-cut loops <a  href="/contests/magnatune/loops">all available for download</a> and remixing.</p>
<h2  style="font-size:18px;margin:0px;font-weight:normal;">Prizes:</h2>
<p >Magnatune will select the top ten winning entries 
 and will sign the winners to their standard "non-evil" <a  href="http://magnatune.com/info/terms">recording contract</a>. All the winning entries will be included on a Lisa DeBenedictis remix album to be sold commercially. The artists' share (50% from the first sale) will be split between the winners and (of course) the artists they sample. </p>
<p >Enter as many times as you wish, you might end up with <b >two</b> (but no more) tracks on the final album which will have the top 10 winning tracks.</p>
<p >Winners will also receive <a  href="http://www.cafepress.com/magnatune/">cool Magnatune gear</a> (T-shirts, sweatshirts, mug, mousepad and more) plus 10 free albums of their choice from the Magnatune catalog.</p>
<h2  style="font-size:18px;margin:0px;font-weight:normal;">No longer accepting entries:</h2>
<p >The deadline for entering the contest has passed. (Official close was at July 31st at 9:00 PM PST)</p>
<h2  style="font-size:18px;margin:0px;font-weight:normal;">Judging:</h2>
<p >Entries will be scored based on the following weighted categories:</p>
<p >Originality - 40% of overall grade<br  />
Creative Re-use and Transformation of Samples from the Magnatune catalog- 30% of overall grade<br  />
Production Quality -30% of overall grade</p>
<h2  style="font-size:18px;margin:0px;font-weight:normal;">Winners:</h2>
<p ><a  href="<?= $A['root-url']?>magnatune/view/contest/winners">View the winners</a>!</p>